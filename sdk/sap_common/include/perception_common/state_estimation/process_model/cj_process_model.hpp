...
class CJProcessModel : public BaseProcessModel {
 public:
    CJProcessModel() = default;
    explicit CJProcessModel(CJProcessModel& model) : BaseProcessModel(model) {}
    ...
    apStatus_t AddPriorConstraints(const KFStatePtr& cur_state_ptr,
                                   ceres::Problem* problem);
    virtual ~CJProcessModel() = default;
};
REGISTER_PROCESSMODEL(CJProcessModel);
