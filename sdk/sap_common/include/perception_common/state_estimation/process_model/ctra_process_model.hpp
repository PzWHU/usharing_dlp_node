...
class CTRAProcessModel : public BaseProcessModel {
 public:
    CTRAProcessModel() : epsilon_(1e-7) {}
    explicit CTRAProcessModel(CTRAProcessModel& model)
    ...
    apStatus_t AddPriorConstraints(const KFStatePtr& cur_state_ptr,
                                   ceres::Problem* problem);
    virtual ~CTRAProcessModel() = default;

 private:
    apStatus_t RemoveRow(Eigen::MatrixXf* matrix, unsigned int rowToRemove);
    apStatus_t RemoveColumn(Eigen::MatrixXf* matrix, unsigned int colToRemove);

 private:
    const float epsilon_;
};
REGISTER_PROCESSMODEL(CTRAProcessModel);
