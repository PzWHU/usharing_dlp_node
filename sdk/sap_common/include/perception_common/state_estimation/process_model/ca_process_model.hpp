...
class CAProcessModel : public BaseProcessModel {
 public:
    CAProcessModel() = default;
    explicit CAProcessModel(CAProcessModel& model) : BaseProcessModel(model) {}
    ...
    apStatus_t AddPriorConstraints(const KFStatePtr& cur_state_ptr,
                                   ceres::Problem* problem);
    virtual std::string Name() const { return "CA_ProcessModel"; }
    virtual const ProcessModelType Type() { return ProcessModelType::CA; }
    apStatus_t Predict(const KFStatePtr& state_in,
                       const KFStatePtr& state_out,
                       const double delta_t);
    virtual ~CAProcessModel() = default;
};
REGISTER_PROCESSMODEL(CAProcessModel);
