...
class CameraRadarMeasureModel : public MeasureModelBase {
 public:
    CameraRadarMeasureModel(const MeasureState& measure_state,
                            const DepthTrackerParameter& depth_smooth_param);

    explicit CameraRadarMeasureModel(
        const CameraRadarMeasureModel& camera_measure_model);

    virtual ~CameraRadarMeasureModel() = default;
