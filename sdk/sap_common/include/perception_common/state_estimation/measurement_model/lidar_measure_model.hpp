...
class LidarMeasureModel : public MeasureModelBase {
 public:
    LidarMeasureModel(const MeasureState& measure_state,
                      const DepthTrackerParameter& depth_smooth_param);

    explicit LidarMeasureModel(const LidarMeasureModel& lidar_measure_model);

    virtual ~LidarMeasureModel() = default;
