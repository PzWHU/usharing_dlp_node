...
class CameraAccelerationMeasureModel : public MeasureModelBase {
 public:
    CameraAccelerationMeasureModel(
        const MeasureState& measure_state,
        const DepthTrackerParameter& depth_smooth_param);

    explicit CameraAccelerationMeasureModel(
        const CameraAccelerationMeasureModel& camera_measure_model);

    virtual ~CameraAccelerationMeasureModel() = default;
