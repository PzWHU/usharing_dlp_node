...
class CameraLateralVelocityMeasureModel : public MeasureModelBase {
 public:
    CameraLateralVelocityMeasureModel(
        const MeasureState& measure_state,
        const DepthTrackerParameter& depth_smooth_param);

    explicit CameraLateralVelocityMeasureModel(
        const CameraLateralVelocityMeasureModel& camera_measure_model);

    virtual ~CameraLateralVelocityMeasureModel() = default;
