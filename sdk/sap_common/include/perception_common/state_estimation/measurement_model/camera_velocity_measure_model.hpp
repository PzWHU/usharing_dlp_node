...
class CameraVelocityMeasureModel : public MeasureModelBase {
 public:
    CameraVelocityMeasureModel(const MeasureState& measure_state,
                               const DepthTrackerParameter& depth_smooth_param);

    explicit CameraVelocityMeasureModel(
        const CameraVelocityMeasureModel& camera_measure_model);

    virtual ~CameraVelocityMeasureModel() = default;
